...
print("Que é o processo do acaso. Mesmo com tudo muito bem ajustado, por conta da arquitetura Transformer, baseada em probabilidades de tokens, o acaso vai influenciar na execução da escolha probablistíca de tokens, logo afeta na geração das respostas do prompt.")
print("Por isso, pode gerar coisas aleatórias ou imprevistas.")

print("Tiktoken é a biblioteca (criada pela Open AI) utilizada para contagem de tokens, o sistema de previsão, baseia-se em cálculos probablísticos.")
print("\n Características da preparação da IA:")

print("\n Respostas anteriores incluídas: Se trata do que o azure (via playground)vai colocar na API (Basicamente um histórico, dando contexto útil e adaptável, já que dá para escolher até quantas vão ser usadas para registro/contexto, que utiliza a quantia estabelecida na resposta máxima); \n Temperatura: escolha randômica das opções (token) e, em conjunto com Top P, a 'criatividade e variedade' da IA \n Top P: Quantas opções de resposta (token) e, junto da Temperatura, a 'criatividade e variedade' da IA  \n Resposta máxima: Define a quantidade máxima de tokens que podem ser utilizados na resposta  \nPenalidade por Frequência: punição, com redução das chances de um token, por ter repetido \n Penalidade por presença: A mesma coisa da por frequência, a diferença é que age na repetição de tópico (ou seja, para evitar repetir assunto)  \nSystem Message: Instruções, contexto e 'Modus Operandi' pro modelo (modela como ele vai agir, para que e diretrizes principais)  \nStop:onde vai parar (com quantos tokens, vai parar)")
...
print("\n Logo, uma temperatura baixa, respostas previsíveis, determinísticas. Uma temperatura mais alta, gera mais aleatoriedade, caos, até mesmo falta de sentido. Low Top P gera uma gama de possiveis respostas menor (em quantidade), Já o Top P alto, deixa um espectro maior de respostas possíveis. ")
...
print("Frequence penalty é o quanto vai punir repetição de token(termo), ou seja, age a favor da variedade vocabular. Enquanto a presence penalty, é punir por diversidade, ou seja, se desvia demais dos temas ou não. Uma P.Penal muito alta, vai evitar fugir demais do tema, já uma baixa, vai variar muito. ")

print("Multimodal: Esses sitemas possuem capacidades além de um único modal de operação. Um exemplo é o famoso DALL-E, o gerador de imagens da Open AI. Ele depende de critérios de especificação no comando: Clareza (o que se quer), especificidade (como se quer, onde usar, para que usar), contexto (detalhes do que estará na imagem) e estrutura (estruturar as informações anteriores). Todo sistema de IA, em seu uso; em cada um dos seus modais de funções, possuem seus requerimentos básicos de funcionamento, conforme cada modelo/modal de função.")

print("Aplicações: no curso, o professor Pablo usa como 'verificador de notícias', como chat da DIO no VScode, e o DALL-E, para ilustrar o sistema de geração de imagens. Existem muito mais formas de utilizar, claro. O único problema por trás, é o consumo de tokens e o preço gasto no uso de tokens.")
